...
const Login = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/auth/oauth-status`)
      .then(res => res.json())
    ...
    <div className="w-full min-h-screen flex items-center justify-center relative overflow-hidden bg-zinc-900">
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl shadow-2xl rounded-2xl p-8 border border-white/40">
          ...
          {/* Only show OAuth option if configured */}
          {oauthEnabled && (
            <>
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full h-11 bg-white hover:bg-slate-50 border-2 border-slate-200 hover:border-slate-300 transition-all duration-300 group"
                onClick={() => {
                  window.location.href = `${import.meta.env.DEV ? 'http://localhost:5002' : ''}/api/auth/google`;
                }}
              >
